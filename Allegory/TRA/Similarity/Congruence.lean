...
`Allegory/TRA/Similarity/HoweExtension.lean`. This file is currently dormant
(its body is commented out) pending a full proof skeleton; the dependencies and
imports it will re-activate are kept in the comment block below for future
restoration.
...
--     bracketRel (tilde a) a ≫ c ≤ c ≫ substitution a c := by
--   -- deps: Harmony.harmony, Compatible, bracketRel, tilde
--   sorry

-- /-- GCP step (Lemma 34 proof, key application of GCP O3): specialised to
-- `x := opHowe a` to push `b` past `⟨tilde (a§), a§⟩`. -/
-- theorem keyLemma_gcp_step {X : A} (a b : X ⟶ X)
--     (_hGCP : GCP b) (_hCompat : Compatible (opHowe a)) :
--     bracketRel (tilde (opHowe a)) (opHowe a) ≫ b
...
-- `openExt b ≫ a§ ≤ a§ ≫ openExt b`, the commutation used in the
...
--   -- deps: keyLemma_harmony_step, howe_fixed_point, opHowe_compatible, opHowe_quasi_transitive, BSimulation, simulationFunctional, openExt, box_le_self, closed_iff_eq_box
--   sorry
...
--   -- deps: keyLemma_open_step, keyLemma_harmony_step, keyLemma_gcp_step, opHowe, opHowe_reflexive, opHowe_substitutive, opHowe_compatible, opHowe_quasi_transitive, leftResidual, simulationFunctional, BSimulation, openExt, box_le_self, closed_iff_eq_box, GCP, Harmony.harmony, substitution_monotonicity_left, substitution_composition_left
--   sorry
