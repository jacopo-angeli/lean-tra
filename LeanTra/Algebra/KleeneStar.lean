...
/-!
# Kleene star in a unital involutive quantale

Development of the Kleene star in a unital involutive quantale, obtained
as the least fixed point of `x ↦ 1 ⊔ a * x` via Knaster–Tarski. The file
splits into two sections: Section A (no involution) supplies `star` and
its basic laws; Section B adds the involution and proves that converse
commutes with the star. Provides the abstract-rewriting vocabulary
consumed by `Metatheory/Confluence/Diamond.lean`.

The file is SRA-free: nothing here refers to `SRA`, `varDiag`,
substitution, or the compatible refinement. Section A also avoids the
involution, using only `Monoid`, `CompleteLattice`, and `IsQuantale`.
...
/-- Kleene star: the least solution of `x = 1 ⊔ a * x`, obtained as the
least fixed point of `starStep` via Knaster–Tarski. -/
...
/-- Transitivity of the star: `a∗ * a∗ ≤ a∗`. -/
...
/-- Right-handed post-multiplication: `a∗ * a ≤ a∗`. -/
...
the right-handed Kleene recursor `x ↦ 1 ⊔ x * a`. -/
...
/-- Sandwich lemma: if `a ≤ b ≤ a∗`, then `b∗ = a∗`. -/
...
/-- Converse commutes with star: `(a∗)ᵒ = (aᵒ)∗`. -/
@[simp]
